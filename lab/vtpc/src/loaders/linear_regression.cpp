...
RegressionResult run_cpu_workload(std::size_t sample_size) {
  ...
  const double residual_sum_squares =
      ...
      slope * slope * sum_xx;
  const double non_negative_sse =
      residual_sum_squares < 0.0 ? 0.0 : residual_sum_squares;
...
RegressionParseOutcome parse_regression_arguments(
    int argc, char** argv, RegressionConfig& config
) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (argument == "--repeats") {
      ...
      try {
        ...
        throw std::invalid_argument(
            std::string("Неверное значение для --repeats: ") + ex.what()
        );
      }
      continue;
    }
    if (argument == "--samples") {
      ...
      try {
        ...
        if (static_cast<unsigned long long>(value) >
          ...
          throw std::invalid_argument(
              "Размер выборки превышает допустимый предел"
          );
        }
        config.samples = static_cast<std::size_t>(value);
      } catch (const std::exception& ex) {
        throw std::invalid_argument(
            std::string("Неверное значение для --samples: ") + ex.what()
        );
      }
      continue;
    }
...
RegressionRunStats run_linear_regression(const RegressionConfig& config) {
  ...
  stats.estimated_total_duration =
      warmup.duration * static_cast<double>(config.repeats);
  stats.actual_duration = timespec_to_seconds(start, end);
  stats.last_result = last_result;
  stats.total_samples =
      config.samples * static_cast<std::size_t>(config.repeats);
  stats.sink_value = g_cpu_sink;
  return stats;
}

void print_regression_summary(
    const RegressionConfig& config, const RegressionRunStats& stats
) {
  std::cout << "CPU-нагрузчик" << std::endl;
  std::cout << "Повторов: " << config.repeats
            << ", точек на повтор: " << config.samples << std::endl;
  std::cout << std::fixed << std::setprecision(6)
            << "Оценка времени выполнения: ~" << stats.estimated_total_duration
            << " сек" << std::endl;
  std::cout << "Фактическая длительность: " << stats.actual_duration << " сек"
            << std::endl;
  ...
  std::cout << "Среднеквадратичная ошибка: " << stats.last_result.mse
            << std::endl;

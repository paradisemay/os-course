...
std::chrono::milliseconds parse_duration(std::string_view argument) {
  ...
  if (!iss || seconds < 0.0) {
    throw std::invalid_argument(
        "ожидалось неотрицательное значение секунд для --duration"
    );
  }
...
std::size_t parse_threads(std::string_view argument) {
  ...
  if (!iss || threads <= 0) {
    throw std::invalid_argument(
        "значение --threads должно быть положительным целым числом"
    );
  }
  const auto max_threads = thread_limit();
  if (static_cast<std::size_t>(threads) > max_threads) {
    std::ostringstream message;
    message << "значение --threads должно быть в диапазоне от 1 до "
            << max_threads;
...
ParseOutcome parse_arguments(
    int argc, char* argv[], bool allow_thread_override, LoaderConfig& config
) {
  config = LoaderConfig{};
  if (allow_thread_override) {
    unsigned int hw_threads = std::thread::hardware_concurrency();
    config.threads =
        hw_threads == 0 ? 2U : static_cast<std::size_t>(hw_threads);
  }
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (argument == "--duration") {
      if (i + 1 >= argc) {
        throw std::invalid_argument(
            "после --duration требуется значение в секундах"
        );
      }
    ...
    if (argument == "--threads") {
      if (!allow_thread_override) {
        throw std::invalid_argument(
            "--threads доступно только для многопоточной версии загрузчика"
        );
      }
      if (i + 1 >= argc) {
        throw std::invalid_argument(
            "после --threads требуется положительное целое число"
        );
      }
    ...
    std::ostringstream message;
    message << "неизвестный аргумент '" << argument
...
void print_usage(const char* program_name, bool allow_thread_override) {
  std::cout << "Использование: " << program_name << " [параметры]\n"
            << "  --duration <секунды>  продолжительность нагрузки (по "
               "умолчанию 5 секунд)\n";
  if (allow_thread_override) {
    std::cout << "  --threads <число>    количество потоков (по умолчанию "
                 "число потоков CPU, максимум "
              << thread_limit() << ")\n";
  }
...
void run_cpu_load(const LoaderConfig& config) {
  ...
  std::cout << std::fixed << std::setprecision(3) << "CPU load finished in ~"
            << elapsed << " с (" << config.threads << " поток(ов))"
            << std::endl;

  if (config.verbose) {
    std::cout << "Всего итераций: "
              << total_iterations.load(std::memory_order_relaxed) << std::endl;
    for (std::size_t index = 0; index < per_thread.size(); ++index) {
      std::cout << "  Поток " << index << ": " << per_thread[index]
                << " итераций" << std::endl;
    }
  }
}

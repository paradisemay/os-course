...
private:
    void Cleanup() {
        if (fd_ != -1) {
            if (close(fd_) == -1) {
                std::cerr << "Предупреждение: close: " << std::strerror(errno) << std::endl;
            }
            fd_ = -1;
        }
        if (!path_.empty()) {
            if (unlink(path_.c_str()) == -1) {
                std::cerr << "Предупреждение: unlink: " << std::strerror(errno) << std::endl;
            }
            path_.clear();
        }
    }
...
void PrintUsage(const char *program) {
    std::cout << "Использование: " << program
              << " [--repeats N] [--cpu-ops COUNT] [--disk-size BYTES]" << std::endl;
}
...
std::string FormatBytes(unsigned long long bytes) {
    ...
    oss << std::fixed << std::setprecision(2) << value << ' ' << suffixes[suffix_index];
    return oss.str();
}
...
void RunCpuWork(std::size_t operations) {
    ...
    for (std::size_t i = 0; i < operations; ++i) {
        acc = std::sin(acc + static_cast<double>((i % 991) + 1) * 0.001) * 0.99991 + 1.0;
        if (acc > 10.0) {
            acc -= 9.0;
        }
    }
...
double MeasureIteration(int fd, std::vector<char> &buffer, std::size_t cpu_ops, std::size_t disk_bytes) {
    timespec start {};
    timespec end {};
...
Options ParseOptions(int argc, char **argv) {
    Options options{.repeats = kDefaultRepeats,
...
int main(int argc, char **argv) {
    try {
        ...
        std::string path = std::string("/tmp/mixed_loader_") + std::to_string(getpid()) + ".dat";
        ...
        std::cout << "Повторов: " << options.repeats << ", CPU-операций/повтор: "
                  << options.cpu_ops << ", дисковых байт/повтор: "
                  << FormatBytes(options.disk_bytes) << std::endl;
        std::cout << "Файловая система: " << fs_info << std::endl;

        const double warmup =
            MeasureIteration(file_guard.fd(), buffer, options.cpu_ops, options.disk_bytes);
        std::cout << std::fixed << std::setprecision(6)
                  << "Оценка времени выполнения: ~" << warmup * options.repeats << " сек" << std::endl;
        ...
        const unsigned long long total_cpu_ops = static_cast<unsigned long long>(options.cpu_ops) *
        ...
        std::cout << "Дисковые данные суммарно: " << FormatBytes(total_disk_bytes) << std::endl;
        std::cout << std::setprecision(3)
                  << "Средняя доля CPU-операций: " << total_cpu_ops / elapsed << " оп/с" << std::endl;
        std::cout << std::setprecision(3)
                  << "Средний дисковый поток: "
                  << (total_disk_bytes / elapsed / (1024.0 * 1024.0)) << " МиБ/с" << std::endl;

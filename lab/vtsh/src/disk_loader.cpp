...
constexpr std::size_t kDefaultFileSize = 16 * 1024 * 1024; // 16 MiB
constexpr std::size_t kDefaultBlockSize = 4096;

void PrintUsage(const char *program) {
    std::cout << "Использование: " << program
              << " [--repeats N] [--file-size BYTES] [--block-size BYTES]" << std::endl;
}
...
double RunDiskIteration(int fd, std::vector<char> &buffer, std::size_t file_size) {
    if (ftruncate(fd, 0) == -1) {
...
int main(int argc, char **argv) {
    try {
        ...
        std::string path = std::string("/tmp/disk_loader_") + std::to_string(getpid()) + ".dat";
        ...
        std::cout << "Повторов: " << options.repeats << ", размер файла: "
                  << FormatBytes(options.file_size) << ", блок: "
                  << FormatBytes(options.block_size) << std::endl;
        std::cout << "Характеристика файловой системы: " << mount_info << std::endl;

        const double warmup = RunDiskIteration(file.fd(), buffer, options.file_size);
        ...
        const unsigned long long bytes = static_cast<unsigned long long>(options.file_size) *
        ...
        std::cout << std::setprecision(3) << "Средняя пропускная способность: " << throughput
                  << " МиБ/с" << std::endl;
    } catch (const std::exception &ex) {

...
std::uint32_t ParseUnsigned(const std::string &text, const std::string &name) {
    try {
        ...
        throw std::invalid_argument("Неверное значение для " + name + ": " + ex.what());
    }
}

std::uint64_t ParseUnsigned64(const std::string &text, const std::string &name) {
    try {
        unsigned long long value = std::stoull(text);
...
Options ParseOptions(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            if (i + 1 >= argc) {
                throw std::invalid_argument("Отсутствует значение после --direction-prob");
            }
    ...
    if (options.degree >= options.node_count) {
        throw std::invalid_argument("Для k-регулярного графа требуется k < количество вершин");
    }
    if ((static_cast<std::uint64_t>(options.node_count) * options.degree) % 2 != 0) {
        throw std::invalid_argument("Произведение количества вершин и степени должно быть чётным");
    }
    if (options.direction_probability < 0.0 || options.direction_probability > 1.0) {
        throw std::invalid_argument("Вероятность направления должна находиться в диапазоне [0, 1]");
    }
    if (options.start_node >= options.node_count) {
        throw std::invalid_argument("Начальная вершина должна существовать в графе");
    }
    return options;
}
...
struct FileHandle {
    ...
    explicit FileHandle(const std::string &path) {
        ...
        if (fd == -1) {
            throw std::system_error(errno, std::generic_category(), "Не удалось открыть файл");
        }
    }
    ...
    void Write(const void *data, std::size_t size, std::uint64_t offset) {
        ...
        while (remaining > 0) {
            ssize_t written = ::pwrite(fd, ptr, remaining, static_cast<off_t>(offset));
            ++operations;
            if (written == -1) {
                if (errno == EINTR) {
                    continue;
                }
                throw std::system_error(errno, std::generic_category(), "Ошибка записи pwrite");
            }
    ...
    void Read(void *data, std::size_t size, std::uint64_t offset) {
        ...
        std::size_t remaining = size;
        while (remaining > 0) {
            ssize_t received = ::pread(fd, ptr, remaining, static_cast<off_t>(offset));
            ++operations;
            if (received == 0) {
            ...
            if (received == -1) {
                ...
                throw std::system_error(errno, std::generic_category(), "Ошибка чтения pread");
            }
...
GraphData GenerateGraph(const Options &options) {
    ...
    const std::uint64_t stub_count = static_cast<std::uint64_t>(options.node_count) *
    ...
    if (stub_count > std::numeric_limits<std::size_t>::max()) {
        throw std::invalid_argument(
            ...
            "поддерживаемый размер контейнера. Уменьшите значения --nodes и --degree, "
    ...
    for (std::size_t attempt = 0; attempt < kMaxAttempts && !generated; ++attempt) {
        ...
        for (std::size_t i = 0; i < total_stubs; i += 2) {
            ...
            if (u == v ||
                std::find(adjacency[u].begin(), adjacency[u].end(), v) != adjacency[u].end()) {
                bool swapped = false;
    ...
    if (!generated) {
        throw std::runtime_error("Не удалось сгенерировать k-регулярный граф без петель и кратных рёбер");
    }

    std::vector<std::vector<EdgeDirection>> directions(options.node_count);
    for (std::uint32_t node = 0; node < options.node_count; ++node) {
        directions[node].resize(adjacency[node].size(), EdgeDirection::Bidirectional);
    }
        ...
        for (std::size_t idx = 0; idx < adjacency[u].size(); ++idx) {
            ...
            if (u < v) {
                ...
                if (roll < options.direction_probability) {
                    ...
                    EdgeDirection dir_u = forward ? EdgeDirection::Outgoing : EdgeDirection::Incoming;
                    EdgeDirection dir_v = forward ? EdgeDirection::Incoming : EdgeDirection::Outgoing;
                    auto it_v = std::find(adjacency[v].begin(), adjacency[v].end(), u);
                    std::size_t idx_v = static_cast<std::size_t>(std::distance(adjacency[v].begin(), it_v));
    ...
    const std::uint64_t nodes_bytes = static_cast<std::uint64_t>(options.node_count) *
    ...
    if (stub_count > 0) {
        if (stub_count > std::numeric_limits<std::uint64_t>::max() / sizeof(EdgeRecord)) {
            throw std::invalid_argument(
                "Переполнение при расчёте смещений рёбер. Уменьшите значения --nodes и --degree, "
                "чтобы произведение nodes * degree оставалось в пределах " +
                std::to_string(std::numeric_limits<std::uint64_t>::max() / sizeof(EdgeRecord)));
        }
    ...
    if (base_offset > std::numeric_limits<std::uint64_t>::max() - edges_bytes) {
        throw std::invalid_argument(
            "Суммарный размер таблиц превышает доступный диапазон смещений. Уменьшите "
            "--nodes или --degree, чтобы произведение nodes * degree не превышало " +
            std::to_string(std::numeric_limits<std::uint64_t>::max() / sizeof(EdgeRecord)));
    }
    ...
    for (std::uint32_t node = 0; node < options.node_count; ++node) {
        ...
        current_offset += static_cast<std::uint64_t>(adjacency[node].size()) * sizeof(EdgeRecord);
    ...
    std::uniform_int_distribution<std::uint32_t> target_node_dist(0, options.node_count - 1);
...
void WriteGraph(FileHandle &file, const GraphData &data) {
    ...
    file.Write(data.nodes.data(), data.nodes.size() * sizeof(NodeRecord), nodes_offset);
    file.Write(data.edges.data(), data.edges.size() * sizeof(EdgeRecord),
...
NodeRecord ReadNode(FileHandle &file, const GraphHeader &header, std::uint32_t node_id) {
    std::uint64_t offset = sizeof(GraphHeader) + static_cast<std::uint64_t>(node_id) * header.node_record_size;
    NodeRecord record{};
...
std::vector<EdgeRecord> ReadEdges(FileHandle &file, const NodeRecord &node) {
    ...
    if (!edges.empty()) {
        file.Read(edges.data(), edges.size() * sizeof(EdgeRecord), node.adjacency_offset);
    }
    return edges;
}

bool TraverseAndModify(FileHandle &file, const Options &options, const GraphHeader &header,
                       Stats &stats) {
    ...
    while (!bfs.empty()) {
        ...
        if (node.value == options.target_value) {
            ...
            file.Write(&node, sizeof(NodeRecord),
                       sizeof(GraphHeader) + static_cast<std::uint64_t>(node_id) * header.node_record_size);
...
Stats Run(const Options &options) {
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &gen_start) == -1) {
        throw std::system_error(errno, std::generic_category(), "clock_gettime (start generation)");
    }
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &gen_end) == -1) {
        throw std::system_error(errno, std::generic_category(), "clock_gettime (end generation)");
    }
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &trav_start) == -1) {
        throw std::system_error(errno, std::generic_category(), "clock_gettime (start traversal)");
    }

    TraverseAndModify(file, options, header, stats);

    if (clock_gettime(CLOCK_MONOTONIC, &trav_end) == -1) {
        throw std::system_error(errno, std::generic_category(), "clock_gettime (end traversal)");
    }
...
void PrintReport(const Options &options, const Stats &stats) {
    ...
    std::cout << "  модификация выполнена: " << (stats.modification_success ? "да" : "нет")
              << std::endl;
}
